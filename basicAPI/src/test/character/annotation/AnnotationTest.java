package test.character.annotation;
...
public class AnnotationTest {
    ...
    // 使用注解相当于实例化抽象注解
    @Annotation2(name = "Annotation Test", age = 25, hobbies = {"coding", "testing"}, type = AnnotationTest.class)
    private String annotationField = "This is a field with Annotation2";

    @Test
    public void annotationDemo() throws NoSuchFieldException {
        /**
         * 局部变量的注解不会在运行时保留，无法通过反射获取
         * 将注解引用到类的字段上，并通过反射获取注解信息
         */

        // 通过反射获得name上的注解Annotation2的信息
        Annotation2 annotation = this.getClass().getDeclaredField("annotationField").getAnnotation(Annotation2.class);
        if (annotation != null) {
