...
public class ThreadPool {
        ...
        threads.submit(
            () -> {
              return Thread.currentThread().getName()
                  + " is executing a submitted task"; // submit方法可以返回结果
            });
  ...
  public void ThreadPoolDemo2() {
    ...
    ExecutorService cachedThreadPool =
        Executors.newCachedThreadPool(); // 创建一个可缓存的线程池，线程数不固定，根据需要创建新线程

    ExecutorService singleThreadExecutor =
        Executors.newSingleThreadExecutor(); // 创建一个单线程的线程池，只有一个线程执行任务，当任务提交时，会等待前一个任务完成后再执行下一个任务

    ScheduledExecutorService scheduledThreadPool =
        Executors.newScheduledThreadPool(2); // 创建一个定时任务线程池，线程数为2，可以执行定时任务
    ...
    scheduledThreadPool.scheduleAtFixedRate(
        threadTask, 0, 10, TimeUnit.SECONDS); // 提交一个定时任务，立即执行，然后每10秒执行一次
