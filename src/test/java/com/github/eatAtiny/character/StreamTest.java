...
public class StreamTest {
  ...
  public void streamDemo() {
    ...
    Stream<String> mappedStream =
        filteredStream.map(String::toUpperCase); // 将剩余的字符串转换为大写 这种写法相当于map(s -> s.toUpperCase())
    Stream<String> sortedStream =
        mappedStream.sorted(); // 对结果进行排序 sorted()方法默认是升序排序 如果需要自定义排序，可以传入一个Comparator对象
    Stream<String> distinctStream = sortedStream.distinct(); // 去重，去掉重复的元素
    Stream<String> stream3 =
        distinctStream.peek(System.out::println); // 使用peek方法打印每个元素，peek是一个中间操作，不会终止流
    ...
    // 使用collect将流转换为列表
    //        ArrayList<String> resultList = sortedStream.collect(ArrayList::new, ArrayList::add,
    // ArrayList::addAll); // 这种写法相当于collect(Collectors.toList());
  }
}
